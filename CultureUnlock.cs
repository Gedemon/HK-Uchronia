...
namespace Gedemon.Uchronia
{
	...
	public class CultureUnlock
	{
		...
		public class TerritoryData // data referencing Giant Earth Map territories, to build a territory unlocking list on generated maps
		{
		...
		public class ContinentData // data referencing Giant Earth Map Continents
		{
			public int Index { get; set; }
		...
		public static void PrepareForGiantEarthReference(World currentWorld)
        {
			...
			if (Uchronia.UseShiftedCoordinates())
			...
			Diagnostics.LogWarning($"[Gedemon] [CultureUnlock] Unknown Map detected (width = {width}, height = {height}, wrap = {wrap}, numLandContinents = {numContinents-1}, numTerritories = {numTerritories}) (GiantEarth width = {GiantEarthWidth}, height = {GiantEarthHeight}), UseReferenceCoordinates = {Uchronia.UseReferenceCoordinates()}");
			...
			foreach(ContinentData continent in continentDataList)
			{
					...
					if (distance < bestDistance)
					{
						...
						for (int j = 0; j < numContinents-1; j++)
						{
							...
							if (continentList.Contains(referenceContinent.Index))
							{
								...
								if (Uchronia.UseReferenceCoordinates() || RefMapContinentSizeFromList[j] >= continent.Size)
								{
				...
				else
				{
					...
					if(!Uchronia.UseReferenceCoordinates())
                    {
						MapCanUseGiantEarthReference = false;
						return;
					}
			...
			foreach (KeyValuePair<string, TerritoryData> kvp in TerritoryReferenceGiantEarthMap)
			{
				...
				if (!Uchronia.UseReferenceCoordinates() && MapUtils.AreSameContinentType(territory, territoryData, currentWorld))
				{
		...
		public static bool ValidateGiantEarthReference(World currentWorld)
		{
			...
			if (Uchronia.IsEnabled() && ListMajorEmpireTerritories.Count == 0)
			{
				Diagnostics.LogError($"[Gedemon] Error : TrueCultureLocation.IsEnabled = {Uchronia.IsEnabled()} && listMajorEmpireTerritories.Count = {ListMajorEmpireTerritories.Count}");
				return false;
			}
		...
		public static void OnMapLoaded(World currentWorld)
		{
			...
			{
				Diagnostics.LogWarning($"[Gedemon] [CultureUnlock] Map is a compatible version of the Giant Earth (CurrentMapHash = {CurrentMapHash})");
			}
			else if(Uchronia.CanCreateTCL())
            {
		...
		public static bool ValidateMapTCL()
        {
			...
			if (Uchronia.GetTotalEmpireSlots() >= 10 && Uchronia.GetTotalEmpireSlots() > System.Math.Max(ExtraPositions.Count, ExtraPositionsNewWorld.Count))
			{
				Diagnostics.LogError($"[Gedemon] Error : TotalEmpireSlots ({Uchronia.GetTotalEmpireSlots()}) > ExtraPositions ({System.Math.Max(ExtraPositions.Count, ExtraPositionsNewWorld.Count)})");
				return false;
			}
		...
		public static bool IsCapitalTerritory(string factionName, int territoryIndex, bool any)
		{
			...
			{
				if (Uchronia.KeepOnlyCoreTerritories() && ListMajorEmpireCoreTerritories.TryGetValue(factionName, out List<int> coreList))
				{
					if(coreList.Count > 0)
