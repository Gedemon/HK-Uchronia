					...
					for (int i = 0; i < count; i++)
					{
						...
						if ((settlement.CityFlags & CityFlags.Captured) == 0)
						{
							...
							for (int k = 0; k < count2; k++)
							{
								...
								bool validSettlement = (Uchronia.GetEraIndexCityRequiredForUnlock() > majorEmpire.DepartmentOfDevelopment.CurrentEraIndex + 1 || settlement.SettlementStatus == SettlementStatuses.City || CultureUnlock.IsNomadCulture(majorEmpire.FactionDefinition.name));
